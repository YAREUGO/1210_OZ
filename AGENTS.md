...
2. **Supabase 클라이언트 파일들** (`lib/supabase/`):
   ...
   - `clerk-client.ts`: Client Component용 (useClerkSupabaseClient hook)
     - `@supabase/ssr`의 `createBrowserClient` 사용 (2025년 권장 방식)
   ...
   - `server.ts`: Server Component/Server Action용 (createClerkSupabaseClient)
     - `@supabase/ssr`의 `createServerClient` 사용 (2025년 권장 방식)
     - Cookie 기반 세션 관리 (Next.js 최적화)
     - 서버 사이드에서 Clerk 인증 사용
     - **주의**: `async` 함수이므로 `await` 사용 필수
...
**새로운 AI 컨텍스트가 시작될 때는 반드시 다음 순서로 진행:**

1. **`docs/HANDOVER.md` 먼저 읽기**
   - 이전까지 완료된 작업 전체 파악
   - 현재 진행 상황 확인
   - 다음 단계 작업 이해
   - 주의사항 및 기술 스택 확인
...
2. **관련 문서 확인**
   - `docs/TODO.md`: 작업 체크리스트 확인
   - `docs/PRD.md`: 제품 요구사항 확인 (필요 시)
   - `docs/ERROR_LOG.md`: 이전에 발생한 이슈 확인

3. **작업 진행**
   - HANDOVER.md의 내용을 바탕으로 이전 작업 상기
   - 다음 단계 작업을 HANDOVER.md에 명시된 우선순위대로 진행
   - 작업 완료 시 HANDOVER.md 자동 업데이트

### 작업 완료 시 필수 작업

**작업 단위가 완료될 때마다 반드시:**

1. **`docs/HANDOVER.md` 업데이트**
   - 완료된 작업을 "완료된 작업" 섹션에 추가
   - 다음 단계 작업을 "다음 단계 작업" 섹션에 명시
   - 발생한 이슈는 `docs/ERROR_LOG.md`에 기록

2. **GitHub 커밋 및 푸시**
   - 변경사항을 커밋하기 전에 HANDOVER.md 업데이트 확인
   - 커밋 메시지에 완료된 작업 명시

3. **컨텍스트 리셋 전 확인**
   - HANDOVER.md가 최신 상태인지 확인
