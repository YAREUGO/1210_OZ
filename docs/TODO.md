...
- [x] 통계 데이터 수집
  - [x] `lib/api/stats-api.ts` 생성
    ...
    - [x] 에러 처리 및 재시도 로직
    - [x] 데이터 캐싱 (unstable_cache, revalidate: 3600)
- [x] 통계 요약 카드
...
- [x] 지역별 분포 차트 (Bar Chart)
  - [x] `components/stats/region-chart.tsx` 생성
    ...
    - [x] X축: 지역명 (서울, 부산, 제주 등)
    - [x] Y축: 관광지 개수
    - [x] 상위 10개 지역 표시 (limit prop)
    - [x] 바 클릭 시 해당 지역 목록 페이지로 이동
    - [x] 호버 시 정확한 개수 표시
    - [x] 반응형 디자인
    - [x] 로딩 상태
    - [ ] 다크/라이트 모드 지원 (향후 구현)
    - [ ] 접근성 (ARIA 라벨, 키보드 네비게이션) (향후 구현)
- [x] 타입별 분포 차트 (Donut Chart)
  - [x] `components/stats/type-chart.tsx` 생성
    - [x] recharts 기반 Pie/Donut Chart 구현
    - [x] 타입별 비율 (백분율)
    - [x] 타입별 개수 표시
...
- [x] 페이지 통합
  - [x] `app/stats/page.tsx`에 모든 컴포넌트 통합
    ...
    - [x] 타입별 분포 차트 (하단)
  - [x] 에러 처리 (에러 메시지 표시)
  - [x] 네비게이션에 통계 페이지 링크 추가 (Navbar)
  - [x] 점진적 로딩 (Suspense 사용)
  - [x] 동적 임포트로 번들 크기 최적화

## Phase 5: 북마크 페이지 (`/bookmarks`)
...
- [x] 북마크 목록 페이지
  - [x] `app/bookmarks/page.tsx` 생성
    - [x] 인증된 사용자만 접근 가능 (auth + redirect)
    - [x] 로그인하지 않은 경우 `/sign-in`으로 리다이렉트
  - [x] 북마크 목록 표시
    - [x] 사용자 북마크 목록 조회 (`getUserBookmarks()`)
    - [x] 각 북마크된 관광지 상세 정보 조회 (`getDetailCommon()`)
    - [x] 카드 레이아웃 (TourList 컴포넌트 사용)
    - [x] 빈 상태 처리 (북마크 없을 때)
    - [x] 에러 상태 처리
- [ ] 북마크 관리 기능 (향후 구현)
  - [ ] 정렬 옵션
