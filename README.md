...
이 통합은 Clerk의 세션 토큰에 `"role": "authenticated"` 클레임을 자동으로 추가하여 Supabase의 인증 요구사항을 충족시킵니다.
...
4. 통합이 활성화되면 **"Clerk domain"**이 표시됩니다 (예: `https://your-app-12.clerk.accounts.dev`)
5. 이 **Clerk domain**을 복사하여 메모해두세요 (다음 단계에서 사용)

> **참고**: 이 단계를 통해 Clerk의 세션 토큰에 `"role": "authenticated"` 클레임이 자동으로 추가됩니다.
...
5. **"Clerk"** 선택 (또는 목록에서 선택)
6. **"Clerk domain"** 입력:
   - 3-1 단계에서 복사한 Clerk domain을 붙여넣기
   - 예: `https://your-app-12.clerk.accounts.dev`
7. **"Save"** 또는 **"Add Provider"** 클릭

> **참고**: Supabase는 이제 Clerk의 JWT를 자동으로 검증할 수 있습니다. JWT secret key를 공유할 필요가 없습니다.

**3-3. 통합 확인**

통합이 성공적으로 완료되면:
- Clerk에서 로그인한 사용자의 세션 토큰이 Supabase에서 자동으로 인증됩니다
- Supabase RLS 정책에서 `auth.jwt()->>'sub'`를 사용하여 Clerk 사용자 ID를 확인할 수 있습니다
...
3. 다음 마이그레이션 파일들을 순서대로 실행:
   ...
   **5-2. Instruments 테이블 생성 (예제용)**
   - `supabase/migrations/create_instruments_table.sql` 파일 내용을 복사하여 붙여넣기
   - **"Run"** 클릭하여 실행
   - 이 테이블은 Supabase 공식 문서의 Next.js 퀵스타트 예제를 위한 것입니다

4. 성공 메시지 확인 (`Success. No rows returned`)

**생성되는 테이블:**
- `users`: Clerk 사용자와 동기화되는 사용자 정보 테이블
- `instruments`: Supabase 예제용 악기 테이블 (예제 페이지에서 사용)
...
- `/instruments`: Supabase 공식 문서 예제 (Next.js 퀵스타트)
...
프로젝트에 이미 Clerk 한국어 로컬라이제이션이 적용되어 있습니다. `app/layout.tsx`의 `ClerkProvider`에서 `koKR` locale이 설정되어 있으며, 한국어 에러 메시지도 커스터마이징되어 있습니다.

**적용된 기능:**
- ✅ 모든 Clerk 컴포넌트가 한국어로 표시
- ✅ 한국어 에러 메시지 커스터마이징
- ✅ Tailwind CSS 4 호환성 설정
...
프로젝트를 Vercel에 배포하는 방법은 [Vercel 배포 가이드](./docs/VERCEL_DEPLOY.md)를 참고하세요.
...
**필수 환경 변수:**
- Clerk 인증 관련 (6개)
- Supabase 데이터베이스 관련 (4개)
- 한국관광공사 API (1개)
- 네이버 지도 API (1개)
- 앱 URL (선택 사항, 1개)

자세한 내용은 [Vercel 배포 가이드](./docs/VERCEL_DEPLOY.md)를 확인하세요.

## 추가 리소스

### 프로젝트 문서

- [Vercel 배포 가이드](./docs/VERCEL_DEPLOY.md) - Vercel 배포 상세 가이드
...
- [Clerk + Supabase 통합 가이드 (공식)](https://clerk.com/docs/guides/development/integrations/databases/supabase)
- [Supabase Third-Party Auth 문서](https://supabase.com/docs/guides/auth/third-party/clerk)
